      ...
      {showMenu && (
        <View className="absolute top-12 left-0 bg-white shadow-md rounded-lg w-full z-10">
          {["Low", "Medium", "High"].map(level => (
            <TouchableOpacity key={level} className={`flex-row items-center gap-x-3 p-3 rounded-lg ${priority === level ? "bg-fuchsia-600 text-white" : "bg-white text-gray-800"}`} onPress={() => setPriority(level)}>
